...
export const getWebinarAttendance = async (webinarId: string,
 ...
 try {
  ...
  for (const type of Object.values(AttendedTypeEnum) as AttendedTypeEnum[]) {
   if (type === AttendedTypeEnum.ADDED_TO_CART && webinar.ctaType === CtaTypeEnum.BOOK_A_CALL) continue
   if (type === AttendedTypeEnum.BREAKOUT_ROOM && webinar.ctaType !== CtaTypeEnum.BOOK_A_CALL) continue

   const countItem = attendanceCounts.find((item) => {
    if (webinar.ctaType === CtaTypeEnum.BOOK_A_CALL && type === AttendedTypeEnum.BREAKOUT_ROOM && item.attendedType === AttendedTypeEnum.BREAKOUT_ROOM) {
     return true;
    }
    return item.attendedType === type;
   })

   result[type as AttendedTypeEnum] = {
  ...
  if (options.includeUsers) {
   for (const type of Object.values(AttendedTypeEnum)) {
    if (
     (type === AttendedTypeEnum.ADDED_TO_CART && webinar.ctaType === CtaTypeEnum.BOOK_A_CALL) || (type === AttendedTypeEnum.BREAKOUT_ROOM && webinar.ctaType !== CtaTypeEnum.BOOK_A_CALL)) {
     continue
    }

    const queryType = webinar.ctaType === CtaTypeEnum.BOOK_A_CALL && type === AttendedTypeEnum.BREAKOUT_ROOM ? AttendedTypeEnum.ADDED_TO_CART : type

    if (result[type].count > 0) {
     const attendances = await prismaClient.attendance.findMany({
      where: { webinarId, attendedType: queryType },
      include: { user: true },
      take: options.userLimit, // Limit number of users returned
      orderBy: {
  ...
  return {
   success: false,
   message: "Failed to fetch webinar attendance data"
  }
 }
}
