...
export const createWebinar = async (formData: WebinarFormState) => {
 try {
  ...
  if (!formData.basicInfo.date) {
   return { status: 404, message: "Webinar data is required" };
  }
  ...
  if (combinedDateTime < now) {
   return { status: 404, message: "Webinar date and time can not be in the past" };
  }
  ...
  const webinar = await prismaClient.webinar.create({
   data: {
    ...
    lockChat: formData.cta.lockChat || false,
